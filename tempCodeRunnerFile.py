...
st.write("---")
query = st.text_input(
    "Ask Period Pal a question",
    value="Why do I get cramps before my period?"
)
...
if st.button("Ask"):
    if not query.strip():
        ...
        st.subheader("Retrieved passages (used as knowledge)")
        for i, d in enumerate(docs):
            st.markdown(f"**Doc {i+1} (score {d['score']:.3f})** — {d['source']}")
            st.write(d['text'])

        if use_generator:
            with st.spinner("Generating a clear explanation..."):
            ...
            st.info(
                "Generator disabled — showing retrieved passages only. "
                "Enable 'Use generator model' in the sidebar to get a full explanation."
            )
